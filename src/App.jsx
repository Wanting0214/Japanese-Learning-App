...
const MessageCircle = (props) => <IconBase icon="💬" {...props} />; // 用於會話
...
const lessonsData = [
  ...
  },
  {
    ...
    grammar: [
      ...
      { id: 2, title: "Vませんか", explanation: "禮貌地勸誘或邀約。", examples: [{ jp: "一緒に京都へ行きませんか。", cn: "要不要一起去京都？" }] },
    ],
    quiz: [
...
const appendixData = {
  numbers: {
    ...
    items: [
      ...
      { jp: "1000", kana: "せん", romaji: "sen" },
      { jp: "10000", kana: "まん", romaji: "man" },
    ]
  },
  time: {
    title: "時間 (Time)",
  ...
  timeWords: {
    ...
    tables: [
      {
        title: "天與時段",
        headers: ["前天", "昨天", "今天", "明天", "後天"],
        rows: [
          { label: "日子", cells: [{ jp: "一昨日", kana: "おととい" }, { jp: "昨日", kana: "きのう" }, { jp: "今日", kana: "きょう" }, { jp: "明日", kana: "あした" }, { jp: "明後日", kana: "あさって" }] },
        ]
      }
    ]
  },
  calendar: {
    title: "日期與星期",
    icon: "🗓️",
    description: "注意4、7、9月及特殊日期。",
    weekdays: [
       { label: "日", kanji: "日曜日", kana: "にちようび" },
       { label: "一", kanji: "月曜日", kana: "げつようび" },
    ],
    months: [],
    days: []
  },
  counters: {
    title: "量詞 (Counters)",
    icon: "📦",
    description: "1, 3, 6, 8, 10 容易發生音變。",
    groups: [
      {
        label: "通用・人物",
        items: [
           { unit: "～つ (個)", list: [{ num: 1, val: "ひとつ" }, { num: 3, val: "みっつ" }] }
        ]
      }
    ]
  },
              ...
              <div className="flex-1 space-y-1">
                <div className="flex items-start justify-between">
                  ...
                  <button 
                    ...
                    className="ml-2 p-1.5 rounded-full text-indigo-400 hover:text-indigo-600 hover:bg-indigo-50 transition-colors flex-shrink-0 opacity-100 sm:opacity-0 sm:group-hover:opacity-100"
                    title="播放整句"
                  >
                    🔊
                  </button>
...
const App = () => {
  ...
  const [activeTab, setActiveTab] = useState('vocab'); // 'vocab', 'grammar', 'quiz', 'conversation'
  ...
  const tabs = [
    { id: 'vocab', label: '單字表', icon: <BookOpen size={18} /> },
    { id: 'conversation', label: '會話', icon: <MessageCircle size={18} /> },
    { id: 'grammar', label: '文法解說', icon: <GraduationCap size={18} /> },
    { id: 'quiz', label: '練習測驗', icon: <CheckCircle size={18} /> },
  ];
